...
# Scatter plot to visualize Spending Score vs Visit Frequency
ggplot(data, aes(x = SpendingScore, y = VisitFrequency)) +
  geom_point(size = 3, alpha = 0.7) +
  labs(title = "Spending Score vs Visit Frequency Distribution",
       x = "Spending Score",
       y = "Visit Frequency") +
  theme_minimal()

# Select relevant columns for clustering
model_data <- data[, c("SpendingScore", "VisitFrequency")]
head(model_data)
...
# Plot clusters
ggplot(data, aes(x = SpendingScore, y = VisitFrequency, color = Cluster)) +
  stat_ellipse(aes(fill = Cluster), geom = "polygon", alpha = 0.4) +
  geom_point(size = 3, alpha = 0.8) +
  labs(title = "Spending Score vs Visit Frequency Clustering",
       x = "Spending Score",
       y = "Visit Frequency") +
  theme_minimal()
